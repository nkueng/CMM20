...
template<typename T> vector<T> concat(const vector<T> &vector1, const vector<T> &vector2) { vector<T> ret = vector1; ret.insert(ret.end(), vector2.begin(), vector2.end()); return ret; }
...
template<typename T> T loop_access(const vector<T> &v, int i) { return v[i % v.size()]; }; 
template<typename T> T clamp_access(const vector<T> &v, int i) { return v[std::min(std::max(i, 0), int(v.size() - 1))]; };
...
NVGcolor DARK_ORANGE = nvgRGB(253 / 2, 151 / 2,  31 / 2);
...
int min_element_index(const vector<double> &v) { return std::min_element(v.begin(), v.end()) - v.begin(); }; 
double min_element(const vector<double> &v) { return *std::min_element(v.begin(), v.end()); };
double max_element(const vector<double> &v) { return *std::max_element(v.begin(), v.end()); };
