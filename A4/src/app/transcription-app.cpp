...
public:
	...
	double log_c_pos = 3.5; double log_c_pos_min = -2.; double log_c_pos_max = 6.;
	double log_c_vel = 3.5; double log_c_vel_min = -2.; double log_c_vel_max = 6.;
	double log_c_reg = -2.; double log_c_reg_min = -6.; double log_c_reg_max = 6.;
	double log_c_x0 = 3.; double log_c_x0_min = -6.; double log_c_x0_max = 6.;
	double log_c_physics = 4.5; double log_c_physics_min = -6.; double log_c_physics_max = 6.;
	...
	virtual double evaluate(const VectorXd &stack) const {
		...
		// --
		double O = 0.; 
		// objective on final position
		O += pow(10., log_c_pos) * (x_prime - x.back()).squaredNorm();
		// objective on final velocity
		O += pow(10., log_c_vel) * v.back().squaredNorm();
		// soft constraint on physics
		for (int i = 0; i < (x.size() - 1); ++i)
		{
			// v_k = 1/h * (x_{k+1} - x_k)
			O += pow(10., log_c_physics) * (h * v[i] - (x[i + 1] - x[i])).squaredNorm();
			// F_k/m = 1/h * (v_{k+1} - v_k)
			O += pow(10., log_c_physics) * (h * u[i] / m - (v[i + 1] - v[i])).squaredNorm();
		}
		// soft constraint to enforce equality of initial guess x_0
		O += pow(10., log_c_x0) * (x[0] - x0).squaredNorm();
		// soft constraint to enforce equality of initial guess v_0
		O += pow(10., log_c_x0) * (v[0] - v0).squaredNorm();
		// regularizer on optimization variables
...
public:
	...
	void resetOptimization() {
		...
		for (auto &_ : x_curr ) { _.setZero(); }
		for (auto &_ : v_curr ) { _.setZero(); }
	}
...
private:
	...
	void drawImGui() override {
		...
		SliderScalar("log_c_vel", ImGuiDataType_Double, &objective.log_c_vel, &objective.log_c_vel_min, &objective.log_c_vel_max); 
...
protected:
	...
	void keyPressed(int key, int mods) override {
		...
		if (key == GLFW_KEY_R) { if (VISUALIZE) { resetVisualization(); } else { resetOptimization(); } } // *
	}
