...
public: 
	// constant gravitational force on ring
	const float F_r = get_Fk(x_sun, Vector2d(0., r_prime)).norm();
	double log_c_pos = -.39; double log_c_pos_min = -1.; double log_c_pos_max = 1.;
	double log_c_vel_dir = 1.; double log_c_vel_dir_min = -2.; double log_c_vel_dir_max = 5.;
	double log_c_vel_val = 1.; double log_c_vel_val_min = -2.; double log_c_vel_val_max = 5.;

	virtual double evaluate(const VectorXd &u_stack) const { 

		// actual trajectory for first K timesteps
		auto u = unstackTraj(u_stack);

		// additional timesteps for predictive control
		int K_prime = 100;
		// add zero control for new timesteps beyond old time horizon
		for (int i = 0; i < K_prime; ++i) u.push_back(Vector2d::Zero());
		// predict physics based on new time horizon
		auto x = get_x(u);
		auto v = get_v(u);
		...
		for (int i = K_prime; i < (K + K_prime); ++i)
		{
			// objective on final position on ring: |x_i - x_sun| = r_prime
			O += pow(10., log_c_pos) * pow((from_(x_sun, x[i]).norm() - r_prime), 2);

			// proved not to work (for K_prime = 0):
			// objective on final velocity direction tangential to ring: (x_i - x_sun) * v_i = 0
			// O += pow(10., log_c_vel_dir) * pow((from_(x_sun, x[i]).dot(v[i])), 2);
			// objective on final velocity value equal to required centripetal force: |v_i|^2 = F_r * r_prime / m
			// O += pow(10., log_c_vel_val) * pow((v[i].squaredNorm() - F_r * r_prime / m), 2);
		}
...
private: 
    ...
    void drawImGui() override { using namespace ImGui; 
		...
		SliderScalar("log_c_vel_dir", ImGuiDataType_Double, &objective.log_c_vel_dir, &objective.log_c_vel_dir_min, &objective.log_c_vel_dir_max); 
		SliderScalar("log_c_vel_val", ImGuiDataType_Double, &objective.log_c_vel_val, &objective.log_c_vel_val_min, &objective.log_c_vel_val_max); 
...
protected: 
	...
	void keyPressed(int key, int mods) override {
		...
		if (key == GLFW_KEY_R) { if (SIMULATE) { resetSimulation(); } else { resetOptimization(); } } // *
	}
