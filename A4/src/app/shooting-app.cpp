...
const int K = 32; // 64; // Number of timesteps (feel free to make smaller if code is running too slowly)
...
public:
	...
	double log_c_reg = -5.; double log_c_reg_min = -9.; double log_c_reg_max = -1.;
	double log_c_x = 1.4; double log_c_x_min = -2.; double log_c_x_max = 2.;
	double log_c_v = 1.2; double log_c_v_min = -2.; double log_c_v_max = 2.;
	...
	virtual double evaluate(const VectorXd &u_stack) const { 
		...
		O += pow(10., log_c_x) * (x_prime - x.back()).squaredNorm();
		// objective on final velocity
		O += pow(10., log_c_v) * v.back().squaredNorm();
		// regularizer on used control input
...
public:
    ShootingApp(int width, int height, const char * title, float pixelRatio = 2.f) : Application(title, width, height) { 
		u_curr.clear(); { for (int _ = 0; _ < K; ++_) { u_curr.push_back(lerp<Vector2d>(double(_) / double(K-1), Vector2d(-1, 1), Vector2d::Zero())); } }
		if (USE_ZERO_INITIAL_GUESS) { resetOptimization(); }
		// --
		O_history.clear(); {
