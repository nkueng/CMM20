...
void Application::init(const char *title, int width, int height, std::string iconPath){
    ...
    ImFont *imFont = io.Fonts->AddFontFromFileTTF(CRL_IMGUI_FONT_FOLDER"/Roboto-Medium.ttf", 15.0f * pixelRatio, &cfg);
...
Application::Application(const char *title, int width, int height, std::string iconPath)
...
void Application::drawFPS(){
    ...
    heightForImGui /= 2;
#endif
    ImGui::SetNextWindowPos(ImVec2(widthForImGui - pixelRatio * 320, 0), ImGuiCond_Always);
    ImGui::SetNextWindowSize(ImVec2(pixelRatio * 320, pixelRatio * 80), ImGuiCond_Always);
    ImGui::SetNextWindowCollapsed(true, ImGuiCond_Once);
    char title[100];
    sprintf(title, "FPS: %.2f###FPS", averageFPS);
    ImGui::Begin(title);
    ImGui::Text("Time spent processing: %.2f%%", averagePercentTimeSpentProcessing);
...
void Application::drawConsole(){
    ...
    for (const Logger::ConsoleText& cText : Logger::consoleOutput) {
        ...
        ImGui::TextColored(color, cText.text.c_str());
    }
    ImGui::End();
}
