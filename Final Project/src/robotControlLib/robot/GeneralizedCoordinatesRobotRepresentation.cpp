...
//TODO: for this implementation, we have chosen not to precompute any quantities (e.g. world coords of joints or joint axes, position/orientations of RBs, etc), trading speed in favor 
//of a conceptually clean implementation. If ever the need will arise, things like RB orientations, positions and rotation axes of joints, etc can be precomputed as soon as the q's get set, 
//which presumably will make things faster, but it will also make this class more difficult to work with as things would always need to be kept in sync properly...
...
int GeneralizedCoordinatesRobotRepresentation::getQIndexForJoint(RBJoint* joint) {
...
//		Quaternion Qerr = getRotationQuaternion(q[jointCoordStartIndex[i]], getQAxis(jointCoordStartIndex[i])).inverse() * state.getJointRelativeOrientation(i);
//		if (Qerr.angularDistance(Quaternion::Identity()) > 1e-8)
//			printf("joint %d: err %15.15lf\n", i, Qerr.angularDistance(Quaternion::Identity()));
	}
...
V3D GeneralizedCoordinatesRobotRepresentation::getQAxis(int qIndex) const {
	if (qIndex>=0 || qIndex <6){
		...
		if (qIndex == 3) return RBGlobals::worldUp;	//y
...
P3D GeneralizedCoordinatesRobotRepresentation::getPivotPointLocalPosition(RobotRB* rb)  const {
...
//		printf("joint %d Q: %lf %lf %lf %lf\n", i, jointOrientation.w(), jointOrientation.x(), jointOrientation.y(), jointOrientation.z());
//		double tmpVal = 0;
//		computeRotationAngleFromQuaternion(jointOrientation, getQAxis(jointCoordStartIndex[i] + 0), tmpVal);
...
void GeneralizedCoordinatesRobotRepresentation::projectVectorOnGeneralizedCoordsAxes(const V3D& vec, const V3D& a, const V3D& b, const V3D& c, double& aVal, double& bVal, double& cVal) {
	...
	if (res.norm() > 0.0001)
		std::cout << "Failed to properly compute generalized coordinates for the input :(" << std::endl;
}
...
void GeneralizedCoordinatesRobotRepresentation::projectWorldCoordsValuesIntoGeneralizedSpace(const V3D& linearVal, const V3D& angularVal, const std::vector<V3D>& jointVal, dVector& generalizedCoordinates) {
	resize(generalizedCoordinates, q.size());
	projectVectorOnGeneralizedCoordsAxes(linearVal, getQAxis(0), getQAxis(1), getQAxis(2), generalizedCoordinates[0], generalizedCoordinates[1], generalizedCoordinates[2]);
...
P3D GeneralizedCoordinatesRobotRepresentation::getWorldCoordinates(const P3D& p, RobotRB* rb) {
	V3D offset(p);
...
V3D GeneralizedCoordinatesRobotRepresentation::getWorldCoordinates(const V3D& v, RobotRB* rb) {
	V3D theVector(v);
	int qIndex = getQStartIndexForRB(rb);
...
V3D GeneralizedCoordinatesRobotRepresentation::getVelocityFor(const P3D& p, RobotRB* rb) {
	//pDot(q) = dp/dq * qDot
	Matrix dpdq;
	dVector res;
...
V3D GeneralizedCoordinatesRobotRepresentation::getAngularVelocityFor(RobotRB* rb) {
	//w(q) = dR/dq * qDot
	Matrix dRdq;
	dVector res;
...
Quaternion GeneralizedCoordinatesRobotRepresentation::getWorldRotationForQ(int qIndex) {
	...
	//2 here is the index of the first translational DOF of the root -- these dofs do not contribute to the orientation of the rigid bodies...
	while (qIndex > 2) {
...
Quaternion GeneralizedCoordinatesRobotRepresentation::getRelOrientationForQ(int qIndex) {
	if (qIndex < 3)
		return Quaternion::Identity();
	return getRotationQuaternion(q[qIndex], getQAxis(qIndex));
}

//this is a somewhat slow function to use if we must iterate through multiple rigid bodies...
...
Quaternion GeneralizedCoordinatesRobotRepresentation::getOrientationFor(RobotRB* rb) {
	//	return Quaternion();
/*
	if (rb->pJoint == nullptr)
...
void GeneralizedCoordinatesRobotRepresentation::compute_dpdq(const P3D& p, RobotRB* rb, Matrix &dpdq) {
	resize(dpdq, 3, q.size());
	dpdq.fill(0);

	//todo: this bit of code (and other jacobians/higher order derivatives) can probably be made faster if ever they become the bottleneck...
	//seems like it would be easy enough to precompute orientations/joint positions/offsets in a first pass from the bottom up, instead of always recomputing them for each j...
	//after all, we know that dpdq_i is the cross product of the vector from pos of q_i to p (all in world coords) with the rotation axis of q_i (in world coords)...
	...
	while (loopIndex > 2) {
		...
		//compute the offset from the location of joint qIndex to point p - if these quantitites were known in world coordinates, 
		//then it would be clear we wouldn't need the loop
		...
		//and now bring this vector to world coordinates, since the step above was computed in the coordinate frame of joint q.
		//again, if RB orientations were precomputed, we could just read it off now...
		while (qIndex > 2) {
...
//computes the jacobian dp/dq that tells you how the world coordinates of p change with q. p is expressed in the local coordinates of rb
void GeneralizedCoordinatesRobotRepresentation::compute_dvdq(const V3D& v, RobotRB* rb, Matrix &dvdq) {
...
void GeneralizedCoordinatesRobotRepresentation::estimate_linear_jacobian(const P3D& p, RobotRB* rb, Matrix &dpdq) {
...
void GeneralizedCoordinatesRobotRepresentation::estimate_linear_jacobian(const V3D& v, RobotRB* rb, Matrix &dvdq) {
...
bool GeneralizedCoordinatesRobotRepresentation::test_linear_jacobian(const P3D& p, RobotRB* rb) {
...
//	print("../out/dpdq_estimated.mat", dpdq_estimated);
	...
	for (int i = 0; i<dpdq_analytic.rows(); i++)
		for (int j = 0; j<dpdq_analytic.cols(); j++) {
			...
			if (fabs(err) > 0.0001) {
				std::cout << "Error at: " << i << " " << j << ": analytic: " << dpdq_analytic(i, j) << " estimated " << dpdq_estimated(i, j) << " error: " << err << std::endl;
				error = true;
			}
		}
...
bool GeneralizedCoordinatesRobotRepresentation::test_linear_jacobian(const V3D& v, RobotRB* rb) {
	...
	for (int i = 0; i<dvdq_analytic.rows(); i++)
		for (int j = 0; j<dvdq_analytic.cols(); j++) {
			...
			if (fabs(err) > 0.0001) {
				std::cout << "Error at: " << i << " " << j << ": analytic: " << dvdq_analytic(i, j) << " estimated " << dvdq_estimated(i, j) << " error: " << err << std::endl;
				error = true;
			}
		}
...
int GeneralizedCoordinatesRobotRepresentation::getQStartIndexForRB(RobotRB* rb) {
	int startIndex = 5;
	if (rb->pJoint != nullptr)
...
bool GeneralizedCoordinatesRobotRepresentation::compute_ddpdq_dqi(const P3D& p, RobotRB* rb, Matrix &ddpdq_dqi, int q_i) {
	...
	while (loopIndex > 2) {
		V3D offset(p);
		if (rb->pJoint != nullptr)	offset = V3D(rb->pJoint->cJPos, p);
		int qIndex = startIndex;
...
bool GeneralizedCoordinatesRobotRepresentation::compute_ddvdq_dqi(const V3D& v, RobotRB* rb, Matrix &ddvdq_dqi, int q_i) {
...
void GeneralizedCoordinatesRobotRepresentation::estimate_ddpdq_dqi(const P3D& p, RobotRB* rb, Matrix &ddpdq_dqi, int q_i) {
	resize(ddpdq_dqi, 3, (int)q.size());
...
void GeneralizedCoordinatesRobotRepresentation::estimate_ddvdq_dqi(const V3D& v, RobotRB* rb, Matrix &ddvdq_dqi, int q_i) {
	resize(ddvdq_dqi, 3, (int)q.size());
...
bool GeneralizedCoordinatesRobotRepresentation::test_linear_jacobian_derivatives(const P3D& p, RobotRB* rb) {
	Matrix dpdq_analytic, dpdq_estimated;
	...
	for (int k = 0; k<getDimensionSize(); k++) {
		...
		for (int i = 0; i<dpdq_analytic.rows(); i++)
			for (int j = 0; j<dpdq_analytic.cols(); j++) {
				double err = dpdq_analytic(i, j) - dpdq_estimated(i, j);
				if (fabs(err) > 0.0001) {
					std::cout << "Error when computing ddpdq_dq " << k << " at: " << i << " " << j << ": analytic: " << dpdq_analytic(i, j) << " estimated " << dpdq_estimated(i, j) << " error: " << err << std::endl;
					error = true;
				}
			}
	}
...
bool GeneralizedCoordinatesRobotRepresentation::test_linear_jacobian_derivatives(const V3D& v, RobotRB* rb) {
	Matrix dvdq_analytic, dvdq_estimated;
	...
	for (int k = 0; k<getDimensionSize(); k++) {
		...
		for (int i = 0; i<dvdq_analytic.rows(); i++)
			for (int j = 0; j<dvdq_analytic.cols(); j++) {
				double err = dvdq_analytic(i, j) - dvdq_estimated(i, j);
				if (fabs(err) > 0.0001) {
					std::cout << "Error when computing ddvdq_dq " << k << " at: " << i << " " << j << ": analytic: " << dvdq_analytic(i, j) << " estimated " << dvdq_estimated(i, j) << " error: " << err << std::endl;
					error = true;
				}
			}
	}
...
//computes the angular part of the jacobian that relates changes in the orientation (represented in axis-angle form) of a link to changes in q
void GeneralizedCoordinatesRobotRepresentation::compute_angular_jacobian(RobotRB* rb, Matrix &dRdq) {
	...
	//q rotates the rigid body (grand...child) about its world coordinate axis... these will be the entries of dR/dq...
	while (qIndices.size() > 0){
		int index = qIndices.back();
		Q = Q * getRelOrientationForQ(qIndices.back());
		//this is just the rotation axis in world coordinates, as this is what we are rotating about to get the orientation of the rigid body if everything else is fixed...
...
void GeneralizedCoordinatesRobotRepresentation::estimate_angular_jacobian(RobotRB* rb, Matrix &dRdq){
	resize(dRdq, 3, (int)q.size());
...
bool GeneralizedCoordinatesRobotRepresentation::test_angular_jacobian(RobotRB* rb){
...
//	print("../out/angular_jacobian_estimated.mat", dRdq_estimated);
	...
	for (int i = 0; i<dRdq_analytic.rows(); i++)
		for (int j = 0; j<dRdq_analytic.cols(); j++) {
			...
			if (fabs(err) > 0.0001) {
				std::cout << "Error at: " << i << " " << j << ": analytic: " << dRdq_analytic(i, j) << " estimated " << dRdq_estimated(i, j) << " error: " << err << std::endl;
				error = true;
			}
		}
...
bool GeneralizedCoordinatesRobotRepresentation::compute_dangular_jacobian_dqi(RobotRB* rb, Matrix &ddRdqdqi, int q_i){
...
void GeneralizedCoordinatesRobotRepresentation::estimate_dangular_jacobian_dqi(RobotRB* rb, Matrix &ddRdq_dqi, int q_i) {
...
bool GeneralizedCoordinatesRobotRepresentation::test_angular_jacobian_derivatives(RobotRB* rb){
...
//		print("../out/angular_jacobian_dqi_estimated.mat", ddRdqdq_estimated);
		...
		for (int i = 0; i<ddRdqdq_analytic.rows(); i++)
			for (int j = 0; j<ddRdqdq_analytic.cols(); j++) {
				...
				if (fabs(err) > 0.0001) {
					std::cout << "Error when computing ddRdqdq " << k << " at: " << i << " " << j << ": analytic: " << ddRdqdq_analytic(i, j) << " estimated " << ddRdqdq_estimated(i, j) << " error: " << err << std::endl;
					error = true;
//					exit(0);
				}
			}
...
void GeneralizedCoordinatesRobotRepresentation::computeMassMatrixForRB(RobotRB* rb, Matrix &massMatrix){
	/*
	M = J'McJ
	Mc = [	m 0 0  0
			0 m 0  0
	...
	*/
	...
	Matrix3x3 MoI = rb->getMOI(getOrientationFor(rb).toRotationMatrix());
...
void GeneralizedCoordinatesRobotRepresentation::computeMassMatrix(Matrix &massMatrix){
	resize(massMatrix, q.size(), q.size());
...
void GeneralizedCoordinatesRobotRepresentation::compute_dpdq_dot(const P3D& p, RobotRB* rb, Matrix &dpdq_dot){
	Matrix ddpdq_dqi;
...
void GeneralizedCoordinatesRobotRepresentation::compute_angular_jacobian_dot(RobotRB* rb, Matrix &dRdq_dot){
	Matrix ddRdq_dqi;
...
void GeneralizedCoordinatesRobotRepresentation::computeCoriolisMatrix(RobotRB* rb, Matrix &coriolisMatrix){
    /*
        [w] = [0	0
               0 [Jw*qDot] ]
    */
...
//computes gravity vector g(q) = J' * F_g in terms of generalized coords
void GeneralizedCoordinatesRobotRepresentation::computeGravityTerm(dVector &g) {
	// define gravity vector in world coordinates
	Vector3d gravity;
	gravity << 0., -9.81, 0.;
	// gravity << 0., 0., -9.81;
	...
	// compute gravity for base
	Matrix dpdq;
	compute_dpdq(P3D(0, 0, 0), robot->root, dpdq);
	g += -dpdq.transpose() * robot->root->rbProps.mass * gravity;
	// DEBUG
	// std::cout << "g for base: " << std::endl;
	// std::cout << g << std::endl;

	// compute gravity for all other RBs
	for (uint i = 0; i < robot->jointList.size(); ++i) {
		RobotRB* rb = robot->jointList[i]->child;
		compute_dpdq(P3D(0, 0, 0), rb, dpdq);		
		g += -dpdq.transpose() * rb->rbProps.mass * gravity;
		// std::cout << "dpdq is of size " << dpdq.rows() << "x" << dpdq.cols() << std::endl;
		// std::cout << "mass term " << rb->rbProps.mass << std::endl;
	}
	// DEBUG
	// std::cout << "g for all RBs: " << std::endl;
	// std::cout << g << std::endl;
}
...
void testGeneralizedCoordinateRepresentation(Robot* robot) {
	...
	if (!(robotState1 == robotState2)) {
		std::cout << "TESTING GENERALIZED COORDINATES: robot state is not the same after projection..." << std::endl;
	}

	if ((q1 - q2).norm() > 1e-8) {
		std::cout << "TESTING GENERALIZED COORDINATES: generalized positions (q) are not the same before and after projection. Error: " << (q1 - q2).norm() << std::endl;
...
//	exit(0);
	...
	if ((q1Dot - q2Dot).norm() > 1e-8) {
		std::cout << "TESTING GENERALIZED COORDINATES: generalized velocities (qDot) are not the same before and after projection. Error: " << (q1Dot - q2Dot).norm() << std::endl;
	}
	...
	for (int i = 0; i < robot->getJointCount(); i++) {
		...
		P3D wc1 = robot->getJoint(i)->child->state.getWorldCoordinates(point);
		P3D wc2 = gcrrNew.getWorldCoordinates(point, robot->getJoint(i)->child);
		if (V3D(wc1, wc2).norm() > 1e-8)
			std::cout << "TESTING GENERALIZED COORDINATES: world coordinates of point on rigid body do not match up... error: " << V3D(wc1, wc2).norm() << std::endl;

		wc1 = robot->getJoint(i)->parent->state.getWorldCoordinates(point);
		wc2 = gcrrNew.getWorldCoordinates(point, robot->getJoint(i)->parent);
		if (V3D(wc1, wc2).norm() > 1e-8)
			std::cout << "TESTING GENERALIZED COORDINATES: world coordinates of point on rigid body do not match up... error: " << V3D(wc1, wc2).norm() << std::endl;

		V3D vec = getRandomUnitVector() * 0.2;
		V3D vc1 = robot->getJoint(i)->child->state.getWorldCoordinates(vec);
		V3D vc2 = gcrrNew.getWorldCoordinates(vec, robot->getJoint(i)->child);

		if ((vc1 - vc2).norm() > 1e-8)
			std::cout << "TESTING GENERALIZED COORDINATES: world coordinates of vectors on rigid body do not match up... error: " << (vc1 - vc2).norm() << std::endl;

		vc1 = robot->getJoint(i)->parent->state.getWorldCoordinates(vec);
		vc2 = gcrrNew.getWorldCoordinates(vec, robot->getJoint(i)->parent);

		if ((vc1 - vc2).norm() > 1e-8)
			std::cout << "TESTING GENERALIZED COORDINATES: world coordinates of vectors on rigid body do not match up... error: " << (vc1 - vc2).norm() << std::endl;

		V3D wv1 = robot->getJoint(i)->child->state.getVelocityForPoint_local(point);
		V3D wv2 = gcrrNew.getVelocityFor(point, robot->getJoint(i)->child);
		if ((wv1 - wv2).norm() > 1e-8)
			std::cout << "TESTING GENERALIZED COORDINATES: velocities of point on rigid body do not match up... error: " << (wv1 - wv2).norm() << std::endl;

		wv1 = robot->getJoint(i)->child->state.velocity;
		wv2 = gcrrNew.getVelocityFor(P3D(0,0,0), robot->getJoint(i)->child);
		if ((wv1 - wv2).norm() > 1e-8)
			std::cout << "TESTING GENERALIZED COORDINATES: velocities of rigid body do not match up... error: " << (wv1 - wv2).norm() << std::endl;

		wv1 = robot->getJoint(i)->child->state.angularVelocity;
		wv2 = gcrrNew.getAngularVelocityFor(robot->getJoint(i)->child);
		if ((wv1 - wv2).norm() > 1e-8)
			std::cout << "TESTING GENERALIZED COORDINATES: angular velocities of rigid body do not match up... error: " << (wv1 - wv2).norm() << std::endl;

		Quaternion r1 = robot->getJoint(i)->child->state.orientation;
		Quaternion r2 = gcrrNew.getOrientationFor(robot->getJoint(i)->child);
		if (!sameRotation(r1, r2)) {
			std::cout << "TESTING GENERALIZED COORDINATES: orientations of rigid body do not match up... error: " << r1.w() << " / " << r2.w() << std::endl;
		}

		if (!gcrrNew.test_linear_jacobian(point, robot->getJoint(i)->child))
			std::cout << "TESTING GENERALIZED COORDINATES: linear jacobian (P3D) does not match FD..." << std::endl;

		if (!gcrrNew.test_linear_jacobian(point, robot->getJoint(i)->parent))
			std::cout << "TESTING GENERALIZED COORDINATES: linear jacobian (P3D) does not match FD..." << std::endl;

		if (!gcrrNew.test_linear_jacobian(vec, robot->getJoint(i)->child))
			std::cout << "TESTING GENERALIZED COORDINATES: linear jacobian (V3D) does not match FD..." << std::endl;

		if (!gcrrNew.test_linear_jacobian(vec, robot->getJoint(i)->parent))
			std::cout << "TESTING GENERALIZED COORDINATES: linear jacobian (V3D) does not match FD..." << std::endl;;

		if (!gcrrNew.test_angular_jacobian(robot->getJoint(i)->child))
			std::cout << "TESTING GENERALIZED COORDINATES: angular jacobian does not match FD..." << std::endl;

		if (!gcrrNew.test_linear_jacobian_derivatives(point, robot->getJoint(i)->child))
			std::cout << "TESTING GENERALIZED COORDINATES: linear jacobian derivatives do not match FD..." << std::endl;
		...
		if (!gcrrNew.test_angular_jacobian_derivatives(robot->getJoint(i)->child))
			std::cout << "TESTING GENERALIZED COORDINATES: angular jacobian derivatives do not match FD..." << std::endl;
	}
