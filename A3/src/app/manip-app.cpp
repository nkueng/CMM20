...
bool DRAW_MOUSE_POSITION = true; 
bool THE_SIM_RUNS_TOO_SLOWLY = true; 
double globalTime = 0.; 
...
public:
	...
	virtual double evaluate(const VectorXd &u) const {
        ...
        if (QUESTION_12) {
   //      	// regularize triangles at head and tail of beam
   //      	int counter = 0;
			// for (const auto &tri : sim->triangles) { 
			// 	if (counter <= 5 || (27 <= counter && counter <= 32))
			// 	{
			// 		std::cout << "regularizing triangle: " << counter << std::endl;
			// 		std::cout << "with nodal indices: " << std::endl;
			// 			for (int i = 0; i < 3; ++i)
			// 			{
			// 				std::cout << tri.nodeIndices[i] << std::endl;
			// 			}
			// 		O += pow(10, lambda) * tri.energy(x);
			// 	}
			// 	counter++;
			// }        

			// regularize angular deviation btw handle rotation (theta) and the line connecting the target position points:
			auto I = sim->getFeaturePointsNodalIndices();
			if (I.size() == 2) // tailored to two feature points
			{
				// 1. get angle of line connecting feature points x'
				auto x_prime = get_x_prime();
				auto vec = seg2(x_prime, I[1]) - seg2(x_prime, I[0]);
				auto dx = vec[0];
				auto dy = vec[1];
				...
				// 2. get angle of handles
				auto theta_L = u[4];
				auto theta_R = u[5];

				// std::cout << "theta_L: " << theta_L << ", theta_R: " << theta_R << std::endl;

				// 3. regularize angular deviation
				O += pow(10, lambda) * (pow(theta_L - angle, 2) + pow(theta_R - angle, 2));
			}

			// // regularize linear control of feature points
			// TODO: regularize changes in u
			// O += pow(10, lambda) * (u[0] + u[1] + u[2] + u[3]);
	...
	VectorXd get_DODu(const VectorXd &u) const {
		...
		VectorXd DODu; DODu.setZero(u.size()); {
			...
			if (step == FD_O_of_u) {
				...
				SparseMatrixd dFdu = get_dFdu(u, x);
				MatrixXd dxdu = solve_AX_EQUALS_B(-get_H(u, x), -dFdu);
				DODu += dxdu.transpose() * dOdx;
			}
...
public: 
    ...
    void drawImGui() override { using namespace ImGui; 
        ...
        SliderScalar("Q12: regularizer", ImGuiDataType_Double, &obj.lambda, &obj.lambda_min, &obj.lambda_max);
...
public: 
	...
	Vector2d get_o_() { return .5*Vector2d::Ones(); }
